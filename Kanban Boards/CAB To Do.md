---

kanban-plugin: board

---

## To-Do

- [ ] News
	- [ ] In the news section the mathematical equations should render properly.
	- [ ] Should also render inline image links. 
	- [ ] Update the daily news page. Remove the orange un-necessary wordings.


## Doing



## Sent for Review



## Done

**Complete**


***

## Archive

- [x] Make the first 10 questions ready with solutions attached with it. ✅ 2026-08-19
- [x] Make Notice for next weeks Q&A session ✅ 2026-08-19
- [x] Check the Hardware ✅ 2026-08-14
- [x] Leet Code Randomize ✅ 2026-08-14
- [x] Implement the hat problem #done ✅ 2026-08-14
- [x] Submit paper to sir. #do ✅ 2026-08-07
- [ ] First Gate Assignments
	- [x] Get Solutions and Make pdf for the first week from chapter 1. ✅ 2026-08-09
	- [x] Make poster 📅 2026-08-09 ✅ 2026-09-07
- [x] Discuss the Exercise Questions with RASA. ✅ 2026-09-07

%% kanban:settings
```
{"kanban-plugin":"board","list-collapse":[false,false,false,false],"show-checkboxes":false,"new-card-insertion-method":"append","tag-sort":[{"tag":"#do"},{"tag":"# done"}],"tag-action":"kanban","move-dates":true,"date-format":"DD-MM-YYYY","date-display-format":"DD-MM-Y","show-relative-date":true,"show-view-as-markdown":false,"move-task-metadata":true,"move-tags":true}
```
%%